...
$scriptDir | Out-File -FilePath "$cmdDir\.source" -Encoding utf8 -Force
...
Copy-Item "$scriptDir\skills\flow-update.md" -Destination "$cmdDir\update.md" -Force
Write-Host ""
Write-Host "Flow plugin installed successfully!" -ForegroundColor Green
Write-Host "Commands available: /flow:intro, /flow:init, /flow:spec, /flow:go, /flow:done, /flow:status, /flow:task, /flow:update"
Write-Host ""
